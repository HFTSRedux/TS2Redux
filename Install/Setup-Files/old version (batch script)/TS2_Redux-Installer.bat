...
:print_header
	...
	echo Discord: https://discord.gg/fBnFZBYht5
...
:path_check
	if exist "%gamepath%\Bin64\Homefront2_Release.exe" (
		...
		for /f "skip=1 tokens=* delims=" %%# in ('certutil -hashfile "%gamepath%\Bin64\Homefront2_Release.exe" MD5') do (
			if not defined md5 (
		...
		if "%md5%"=="%homefrontmd5%" (
			echo Game executable check: PASS - Ready to patch
		) else (
			echo WARNING: Your game executable is a different version than what is expected.
			echo The patching process may or may not work for your game version.
			echo If you run into issues, report them on Github or Discord.
			echo.
			echo Game checksum: %md5%
			echo Expected checksum: %homefrontmd5%
			pause.
			pause
		)
...
:do_patch
	...
	set "ts2path=%gamepath%\Bin64\TimeSplitters2.exe"
